...
from art_generation.util import parse_anime_prompts, parse_pony_prompts, safesearch_filter
...
# load model on half precision doesn't affect the results
def load_model(args, load_on_run_all=True, half_precision=True):
    model_map = {
        "sd-v2-1.ckpt": {
        ...
        'midjourney-v2.ckpt': {
            'url': 'https://huggingface.co/dreamlike-art/dreamlike-diffusion-1.0/resolve/main/dreamlike-diffusion-1.0.ckpt'
        },
        'midjourney-v1.ckpt': {
            'url': 'https://huggingface.co/prompthero/openjourney-v2/resolve/main/openjourney-v2.ckpt'
        },
        'inkpunk-diffusion.ckpt': {
            'url': 'https://huggingface.co/Envvi/Inkpunk-Diffusion/resolve/main/Inkpunk-Diffusion-v2.ckpt'
        },
        'analog-diffusion.ckpt': {
            'url': 'https://huggingface.co/wavymulder/Analog-Diffusion/resolve/main/analog-diffusion-1.0.ckpt'
        },
        'linkedin-diffusion.ckpt': {
            'url': 'https://huggingface.co/prompthero/linkedin-diffusion/resolve/main/lnkdn.ckpt'
        }
    }

    # config path
    models_path = './art_generation/'
    ...
    ckpt_path = args.custom_checkpoint_path if args.model_checkpoint == "custom" else f'./{args.model_checkpoint}'
    ckpt_path_2 = f'./art_generation/{args.model_checkpoint}'
    ckpt_valid = True
    if os.path.exists(ckpt_path):
        print(f"{ckpt_path} exists")
    elif os.path.exists(ckpt_path_2):
        ...
        print(f"Attempting to download {args.model_checkpoint}... this may take a while")
        ...
        # write to model path
        with open(os.path.join(models_path, args.model_checkpoint), 'wb') as model_file:
            model_file.write(ckpt_request.content)
    else:
        print(f"Please download model checkpoint and place in {os.path.join(models_path, args.model_checkpoint)}")
        ckpt_valid = False
...
def load_model_from_config(config, ckpt, device, verbose=False, half_precision=True):
    model, m, u = None, None, None
    if ckpt.endswith(".ckpt"):
        print(f"Loading model from {ckpt}")
        if not os.path.exists(ckpt):
            ckpt = os.path.join('./art_generation/', ckpt)
...
def parse_args(args, prompts=[], nprompts=[]):
    ...
    try:
        import langdetect
        if langdetect.detect("".join(prompts)) == "zh-cn" or langdetect.detect("".join(prompts)) == "zh-tw":
            from art_generation.util import chinese_to_english
            prompts = chinese_to_english(prompts)
    except ImportError:
        import subprocess, sys
        subprocess.check_call([sys.executable, "-m", "pip", "install", "langdetect"])
        return parse_args(args, prompts, nprompts)
    ...
    if len(nprompts) == 0:
        ...
        if 'anything' in ckpt:
            ...
            args.nprompts = readLines(f'{folder}analog_diffusion.txt')
        elif 'midjourney-v1' in ckpt:
            args.nprompts = readLines(f'{folder}midjourney_v1.txt')
...
# TODO: upscale image if upscale_ratio > 1

def render_image_batch(args: SimpleNamespace, prompts: list[str] = [], nprompts: list[str] = [], upscale_ratio: int = 1, save_image: bool = True, check_safety=False) -> Tuple[None, Image.Image]:
    ...
    for i, prompt in enumerate(args.prompts):
        args.prompt = prompt
        print(f"\033[92mPrompt {i+1} of {len(prompts)}\033[0m")
        print(f"\033[92m{args.prompt}\033[0m")
